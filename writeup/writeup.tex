...
However, this method is counter-intuitive for a CFR agent(?)
...
\begin{itemize}
	\item Use of two neural networks. The training network is constantly updated in every training iteration, while the target network occasionally synchronises with the training network. The target network stabilises the Q-values used in updating the training.
